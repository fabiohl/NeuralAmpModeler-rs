...
* **Extremely Fast SIMD Inference & Zero-Vtable Dispatch:** Mandatory `x86-64-v3` (AVX2/FMA/BMI2) baseline vectorization with upward-only dynamic dispatch for AVX-512 (architecturally governed by a strict $\ge 12\%$ end-to-end `process()` ROI promotion policy). Static math dispatch is resolved at compile time via the `dispatch_simd!` macro with zero function pointers or vtables, while production neural models operate strictly on native single-precision `f32` weights and buffers with zero vtable overhead. Multi-accumulator ILP (`sum0..sum3` in AVX2, dedicated 32-register `YMM0..YMM31` / EVEX masks in AVX-512 VL256) and tap-major frame-tiling saturate hardware execution ports.
...
| **SIMD Acceleration**            | Mandatory `x86-64-v3` (AVX2/FMA) baseline + upward-only AVX-512 dynamic dispatch (f32)        | Ultra-low CPU usage (< 37 µs SLA contract baseline for WaveNet Std, 7.5 µs for LSTM on AMD Ryzen 7) |
...
* **SIMD Acceleration:** Mandatory AVX2 baseline (`x86-64-v3`) with upward-only AVX-512 dynamic dispatch. All production models execute native single-precision `f32` math (BF16/VNNI is retained for evaluation but is not a guaranteed production acceleration). Architecturally governed by a strict $\ge 12\%$ end-to-end `process()` ROI promotion policy on target hardware. FastMath activations (tanh, sigmoid) via Padé/minimax polynomial approximations, with exact-grade `Standard` mode as default.
...
| [`utils/remote-simd-gate.sh`](utils/remote-simd-gate.sh)   | **Remote SIMD Gating Suite:** Automated harness for running cross-ISA parity validation and latency benchmarks on remote machines/VMs equipped with AVX-512 (AMD Zen 4/5, Intel Xeon), generating structured JSON audit receipts (`target/logs/remote-simd-receipt.json`).                                                                                                                                                                                                                    |
