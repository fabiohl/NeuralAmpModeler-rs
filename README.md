...
| [`utils/verify_no_avx512_release.sh`](utils/verify_no_avx512_release.sh) | **Fail-Closed AVX-512 Absence Certificate:** rebuilds the default release in an isolated `target/cert-release-*` dir (thin-LTO off, SHA-256 logged), then `nam_bin_guard scan` disassembles every ELF member/executable section and flags any instruction whose encoding bytes start with the EVEX `0x62` prefix, plus a defensive `llvm-nm` symbol scan. Any tool or format failure exits 1 — the default build's AVX-512 absence is formally certified, never assumed. |
