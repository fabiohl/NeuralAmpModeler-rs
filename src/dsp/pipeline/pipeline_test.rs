...
mod tests {
    ...
    #[test]
    fn streaming_pipeline_with_streaming_dsp_buffers() {
        use crate::dsp::resampling::StreamingResampleBuffer;
        ...
        let streaming_bufs = StreamingDspBuffers {
            resamp_out_l: &mut resamp_out_l,
            resamp_out_r: &mut resamp_out_r,
            os_in_l: &mut os_in_l,
            os_in_r: &mut os_in_r,
    ...
    #[test]
    fn non_finite_input_is_detected_contained_and_sanitized() {
        use crate::common::spsc::RT_STATUS_NON_FINITE_INPUT_DETECTED;

        let mut input_l = vec![0.05f32; 64];
        let mut input_r = vec![0.05f32; 64];
        input_l[10] = f32::NAN;
        input_r[20] = f32::INFINITY;

        let rt_status = RtStatusFlags::default();
        let (out_l, out_r) =
            run_pipeline_test_with_status(48000, 48000, &input_l, &input_r, 64, false, &rt_status);

        assert!(
            rt_status.check_flag(RT_STATUS_NON_FINITE_INPUT_DETECTED),
            "NaN/Inf input must trigger RT_STATUS_NON_FINITE_INPUT_DETECTED flag"
        );
        for (i, &sample) in out_l.iter().enumerate() {
            assert!(
                sample.is_finite(),
                "Output L sample at {} must be finite, got {}",
                i,
                sample
            );
        }
        for (i, &sample) in out_r.iter().enumerate() {
            assert!(
                sample.is_finite(),
                "Output R sample at {} must be finite, got {}",
                i,
                sample
            );
        }
