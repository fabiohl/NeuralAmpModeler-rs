...
parse_benchmarks() {
    ...
    # Micro DSP benches run DSP_MICRO_BATCH blocks per Criterion sample
    # so timer noise stays below the 2% wall. Report per-block latency here.
    # Canonical source: benches/constants.rs::DSP_MICRO_BATCH, exported to
    # target/bench_constants.env by build.rs (regenerated by any cargo build).
    # Never hard-code the value here.
    local bench_constants_env="target/bench_constants.env"
    local dsp_micro_batch=""
    if [ -f "$bench_constants_env" ]; then
        # shellcheck source=/dev/null
        . "$bench_constants_env"
        dsp_micro_batch="${DSP_MICRO_BATCH:-}"
    fi
    if ! [[ "$dsp_micro_batch" =~ ^[1-9][0-9]*$ ]]; then
        echo "ERROR: cannot resolve DSP_MICRO_BATCH — $bench_constants_env missing or invalid." \
             "Run any 'cargo build'/'cargo bench' in NeuralAmpModeler-rs to regenerate it (build.rs)." >&2
        return 1
    fi
    LC_ALL=C awk -v dsp_micro_batch="$dsp_micro_batch" '
    BEGIN {
        bench = ""
        micro_batch["RT_DSP_Resampler_44k1_to_48k"] = dsp_micro_batch
        micro_batch["RT_DSP_Resampler_96k_to_48k"] = dsp_micro_batch
        micro_batch["RT_DSP_CabSim_IR_Medium"] = dsp_micro_batch
    }
