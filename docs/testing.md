...
| **`dual-mono`**      | Enable dual-mono DSP processing | DSP input/output buffers               | [src/dsp/pipeline/stages/input.rs](../src/dsp/pipeline/stages/input.rs) (Stereo/dual-mono variants)                      |
...
- `gaps`: canonical typed markers detected in the phase log (see the marker grammar below) plus the phase's own gap status; the suite line lists gap phases as `phase_id:STATUS` (and `phase_id:ZERO_TESTS` for a timed phase that passed with zero executed tests).
- `timestamp`: ISO-8601 UTC emission time.

**Typed marker grammar:** every test deviation, bypass, or
...
(cross-ISA opt-in accounting), and — during the transition only — the
...
**Fail-closed receipt invariant:** `overall: PASSED` is only ever
...
### 6.3. Human Release-Certification Roadmap (operator-only)
...
- **ISA parity:** End-to-end cross-ISA determinism via validated `TEST_ISA_OVERRIDE` installs (`set_test_isa_override`/`testing::isa_guard` — safe Rust can never force dispatch to an ISA the host cannot execute). Self-consistency asserts bit-exact output; cross-ISA asserts ESR within calibrated budgets. The AVX-512 path requires the **full `F+VL+BW+DQ` capability matrix** (`avx512f` + `avx512vl` + `avx512bw` + `avx512dq`); any partial subset deterministically falls back to AVX2. Production dispatch is **AVX2 only**; the AVX-512 cross-ISA matrix is a research opt-in (`avx512` feature) parity measurement, and legacy VNNI+BF16 tests remain evaluation checkpoints (`#[ignore]`d in quick); quick covers AVX2 self-consistency + synthetic spectral.
