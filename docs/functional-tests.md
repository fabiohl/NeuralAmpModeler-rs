...
- [ ] **Tier 1.1 Model Loading & Basic Inference:** Load a `.nam` (JSON) and a `.namb` (binary container) model via `load_and_build_model()`. Call `process()` with a 64-sample block of silence. *Expected:* Function returns without panic; output buffer contains finite, non-NaN values.
- [ ] **Tier 1.2 Static Dispatch Coverage:** Load one model from each supported architecture family (WaveNet A1, WaveNet A2, LSTM, ConvNet, Linear). Run a single block of inference per model. *Expected:* All five architectures instantiate and process successfully.
- [ ] **Tier 1.3 Dynamic Model Fallback:** Load a model whose geometry does not match compile-time static profiles (e.g., custom LSTM 3×5 or WaveNet CH=7). *Expected:* Dispatches to the appropriate `Dyn` variant and produces valid output.
- [ ] **Tier 1.4 Corrupted Model Rejection:** Attempt to load a truncated, malformed, or 0-byte `.nam`/`.namb` file. *Expected:* Returns a strongly typed `Err(LoadError)` (see `src/loader/error.rs`) and emits structured diagnostics with a descriptive `NamErrorCode` without panicking.
- [ ] **Tier 1.5 Sample Rate Adaptation:** Load a 48 kHz model. Execute `model.reset(44100, 64)`, process a block, execute `model.reset(96000, 64)`, and process again. *Expected:* Clean state transition without allocation leaks or audio discontinuities.
...
runs the `simd_probe` CLI and records the host SIMD
...
| **`target/logs/phase-defense-scripts.log`**      | `tests-long.sh` (Phase 2)               | Structural invariant checks, QA defenses (`tests/qa_defense.rs`), and dynamic linker symbol export audits (`libm_export_guard`).                                 |
...
| 4   | `perf-check.log`        | `utils/tests-performance-regression.sh --check`                 | pre-approved baseline (see §5.2)         | `target/logs/regression_phase_receipt.jsonl` (+ `target/logs/regression-check.log`) | machine regression verdict (`nam_perf_gate verdict` over Criterion `change/estimates.json`: relative mean-change CI vs the ±5% noise band) against a baseline **produced by an earlier, explicitly approved commit**; exit 0                |
