...
| `golden_wavenet_a2_max.bin`                  | `wavenet_a2_max.nam`                  | **Official real** — KB-A2-MAX known bug (guard TR1.1; meter ~1.69 dB; §4.4.3) | CH=4, cond=8 FiLM; retained for future reopen diagnostics               |
...
>
> - **Official real** — `.nam` model with trained weights, published by the official NAM project (`sdatkinson/NeuralAmpModelerCore`).
> - **Community real** — `.nam` model with trained weights, published by the community (Boss Waza Tube Amp Expander).
> - **Synthetic** — auto-generated/calibrated weights for validation purposes; they do **not** represent real amplifier tones.
> - **C++ reference** — reference vectors generated by upstream C++ (`dsp::ImpulseResponse`); not NAM models.
>
> [!NOTE]
> **Quick summary:** 5 goldens are **official real** (A1-Standard, LSTM Official, A2-Max-disabled,
> Condition-DSP, Official CH=3).
> 9 are **community real** (WaveNet Standard, Lite CH=12, Feather, Nano,
> LSTM 1×16/2×8 — all Boss Waza — plus EVH Stealth 100, Boss BD-2, Slammin Marshall J45,
> all non-distributable).
> 24 are **synthetic** (A2-Full, A2-Lite, A2-FiLM-Full, A2-FiLM-Lite,
...
| `wavenet_a2_max.nam`                  | Official Real    | WaveNet (CH=4, cond=8 FiLM, head1x1)                                     | **Known bug KB-A2-MAX** (fail-closed TR1.1; prod×C++ **SNR ≈ 1.69 dB**; see [cpp_parity_map.md](cpp_parity_map.md) §4.4.3) | Steve Atkinson official example. CC0.                                                     | Permanent known bug. `test_wavenet_a2_max_dispatch_rejected` asserts `Err` (KB-A2-MAX). Golden/meter/live ignored — not CI gates. Unlock: `NAM_A2_MAX_UNLOCK=1` (test/testing only).                                             |
...
- **Nature:** Real NAM models trained by the Boss Waza Tube Amp Expander community. Validates the LSTM architecture with real amplifier weights (1 layer × H=16 and 2 layers × H=8).
...
> [!IMPORTANT]
> **These A2 goldens use synthetic weights, NOT official model weights.** The official A2 model
> `wavenet_a2_max.nam` is **known bug KB-A2-MAX** (fail-closed TR1.1; prod×C++ **SNR ≈ 1.69 dB**;
> [cpp_parity_map.md](cpp_parity_map.md) §4.4.3). Fixture + golden remain for future investigation
> (unlock only via `cfg(test)` / feature `testing` + `NAM_A2_MAX_UNLOCK=1`). The A2 **fast-path**
> does not support FiLM — FiLM is exercised by `WaveNetA2Dyn` with synthetic fixtures
> `wavenet_a2_film_full/lite.nam`. `slimmable_wavenet.nam` **loads** — disclaimer:
> inference-only; without multi-size NAMCore parity claim (NAMCore lacks a slicing API).
>
> **What these goldens validate:** Rust↔C++ numerical parity of the A2 architecture **fast-path**
> (23 layers, K=6/15, LeakyReLU, head_scale=0.02). **What they do NOT validate:** real amplifier
> tones or perceptual fidelity of trained A2 models.
>
> For PM-05 compliance details with real FiLM captures, see §FiLM Fixtures below.
...
> [!NOTE]
> **PM-05 Compliance:** The `WaveNetA2Dyn` engine natively supports FiLM. The
> real model `wavenet_a2_max.nam` is **KB-A2-MAX** (fail-closed; §4.4.3) — fixture/golden
> remain in the repository for future reopening, not as a release gate. Synthetic fixtures
> validate the FiLM engine. See [cpp_parity_map.md](cpp_parity_map.md) §4.4.3.
