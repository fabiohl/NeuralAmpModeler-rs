...
The **single exception** to the "never constructed by the core crate" rule is `E2202` INVALID_CABSIM_PARTITION_SIZE: a DSP constructor-parameter validation error returned by the core crate's own `ConvEngine::new` (a pure DSP constructor, not a host/backend surface) when `partition_size == 0` — preventing a panic on the audio thread during transient host init or config reset.
