...
2. **Upward-Only Dynamic Dispatch:**
   - Because `x86-64-v3` is guaranteed at build time, AVX2 + FMA instructions are used unconditionally across the entire codebase without runtime branching or feature checks (`is_x86_feature_detected!("avx2")` is never queried).
   - Dynamic CPU detection (`SIMD_MATH` via `detect_best_simd()`) operates strictly **upward**: it queries for higher hardware extensions (AVX-512 variants) only. There are no scalar fallback paths or downward branching in the real-time audio hot path.
   - Static dispatch to math implementations (`Avx2Math` / `Avx512Math`) is resolved at compile time via the `dispatch_simd!` macro (see [`src/math/common/mod.rs`](../src/math/common/mod.rs)), matching on the detected ISA and emitting direct, inlined calls per branch without vtables or function pointers. `Avx512VnniBf16` is deprecated and folds into `Avx512Math` (`f32`). `SimdMathConfig` acts solely as descriptive metadata (`instruction_set`, `name`, `is_avx512`).

3. **AVX-512 ROI Policy & Decision Boundaries (Process Gate $\ge 12\%$):**
   - Specialized AVX-512 kernels are subject to a strict return-on-investment (ROI) promotion gate: an AVX-512 path is merged into active production dispatch only if it demonstrates a measured throughput speedup of **$\ge 12\%$** in full, end-to-end `NamModel::process()` execution ($p < 0.05$, evaluated under representative 64-sample buffer quanta at 48 kHz on bare metal or cloud instances) relative to the baseline AVX2 path.
   - Speedup $< 5\%$ results in **DROP** (no duplication; reliance on native `x86-64-v3` AVX2/FMA baseline or LLVM auto-vectorization).
   - Speedup in $5\% \le \Delta < 12\%$ is dropped unless a measurable reduction in real-time latency variance (N=1 jitter) is documented. Isolated matrix-vector (GEMV) microbenchmarks serve strictly as diagnostic indicators.

4. **SIMD Architectural State: VL256 vs. ZMM & Non-Duplication Policy:**
   - **Primary Target (VL256):** The core microarchitecture for AVX-512 in low-latency neural audio is AVX-512 VL256 (`avx512f` + `avx512vl`), utilizing 32 YMM registers (`__m256`). This provides 32 vector registers for loop unrolling and eliminates register spilling without incurring CPU frequency downclocking (thermal throttling) or 512-bit ZMM register padding overhead typical of small neural geometries (channel counts 3, 4, 8, 12, 16).
   - **Strict Non-Duplication Policy ("NO DUPLICATION"):** To prevent code bloat, binary inflation, and maintenance divergence, the project enforces a strict "NO DUPLICATION" policy end-to-end. Sub-routines that are memory-bandwidth bound or where LLVM achieves saturation natively on `x86-64-v3`—specifically Gain, Dither, Pan, Stereo Mixing, CabSim UPOLS FFT, and dynamic non-standard topologies (`LstmModelDyn`, `WaveNetModelDyn`)—utilize unceremoniously and exclusively the unified `x86-64-v3` (AVX2/FMA) baseline, with zero duplication in the `Avx512Math` arm.
   - **AVX-512 Specialized Domains:** AVX-512 specialization is strictly restricted to compute-heavy matrix and convolution kernels:
     1. **LSTM 4-Gate GEMV:** Uses `gemv_4gate_avx512vl` (256-bit EVEX, 32 YMM registers). The AVX-512 LSTM path is intentionally mixed: the GEMV is VL256, while the fused tanh/sigmoid/accumulate steps run on 16 lanes (`__m512`, `lanes=16` in `layer_kernels.rs`) — a deliberate D2 design.
     2. **WaveNet Conv1D (`dot_4x`) & Skip/Residual Accumulate:** Targeted for VL256 with 32 YMM registers and opmasks `%k0..%k7` (currently in production on 16 lanes `__m512` in `dot_f32_avx512.rs` and `wavenet/accumulate/avx512.rs`, with the VL256 EVEX port candidate gated on the cloud $\ge 12\%$ ROI).
     3. **WaveNet `dot_16x`:** Uses native AVX-512 ZMM (`__m512`) where 16 f32 lanes naturally saturate a single 512-bit register.
   - **Measured vs. Aspirational (2026-08 audit):** The `process()` ROI gate (§3) has **not** been measured on AVX-512 hardware yet — no cloud receipt exists. Only the LSTM 4-gate GEMV is a real VL256 port in production; the WaveNet AVX-512 kernels remain ZMM (`__m512`), with the VL256 port deferred until the cloud gate clears §3.
...
5. **L1i Instruction Cache Budget Defense (32 KB Limit):**
   ...
   - Monomorphized dispatch is collapsed to 2 branches (`Avx2Math` and `Avx512Math`), saving ~4.8 KB of `.text` by eliminating redundant VNNI/BF16 monomorphizations.
   - The "~10.22 KB active hot-path working set" figure is **unverified** — no AVX-512 hardware receipt exists for it (2026-08 audit). The static `.text`-size tables in `docs/benchmarks.md` are retained as design intent, not as a measured claim.

6. **Precision & BF16/VNNI Status:**
   - Production neural inference executes strictly with single-precision `f32` weights and activations to preserve bit-exact and float-exact parity against C++ NAMCore.
   - BF16 / VNNI does not offer guaranteed production acceleration for standard `f32` neural models. Half-precision conversion kernels and `Avx512VnniBf16` are retained for evaluation and legacy compatibility, but are not production defaults.
...
| `E2xxx` | Audio / RT — **reserved**           | `E2001` PROCESSING_OVERLOAD, `E2100` AUDIO_INIT_FAILED, `E2101` STREAM_ERROR, `E2200` RESAMPLER_BUILD_FAILED, `E2201` RESAMPLER_CHANNEL_FULL, `E2300` RT_PRIORITY_DENIED, `E2301` CPU_AFFINITY_FAILED — reserved for downstream integrations (e.g., plugin wrappers, standalone hosts, offline renderers); never constructed by the core crate (F-19)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
