...
2. **Dispatch Engine (kept) vs. AVX-512 Promotion (not justified):**
   ...
   - **Retention & isolation strategy:** the dispatch engine and AVX-512/VL256 sources are retained in-tree strictly as research/test artefacts (for re-measurement, `TEST_ISA_OVERRIDE`, and `utils/remote-simd-gate.sh`). They are gated behind the optional, opt-in `avx512` Cargo feature — **clean conditional compilation** (`#[cfg(feature = "avx512")]`): a default build never compiles the AVX-512 sources, so segregation is guaranteed by the compiler and no post-link EVEX scanning/certification is needed or performed.
   - **Production dispatch status:** `detect_best_simd()` resolves to `InstructionSet::Avx2` in default builds. Under the opt-in `avx512` feature it resolves to `Avx512` only when the host supports the **full `F+VL+BW+DQ` capability matrix** (`avx512f` + `avx512vl` + `avx512bw` + `avx512dq`); any partial subset (e.g. F+VL without BW/DQ, common in VMs/hypervisors) falls back to AVX2 — never a `SIGILL` (F-ROB-03). The `dispatch_simd!` macro monomorphizes only `Avx2Math` in default builds, eliminating all AVX-512 kernels and EVEX/ZMM opcodes from the default release `.text` segment. AVX-512 monomorphization is compiled only when `--features avx512` is explicitly supplied at build time for research and benchmarking.
...
5. **L1i Instruction Cache Budget Defense (32 KB Limit):**
   ...
   - Shipping both `Avx2Math` and `Avx512Math` monomorphs in default binaries would undermine the L1i footprint target while AVX-512 is not promoted. Gating AVX-512 behind `cfg(feature = "avx512")` ensures default release binaries monomorphize only `Avx2Math`, eliminating dead AVX-512 code from `.text` while preserving kernel sources for opt-in research. The `.text` surface is therefore a *pure function of the feature flags chosen at build time* — switchable without touching dispatch code, and verifiable with the diagnostic probe (`utils/simd-probe.sh`, see [testing.md](testing.md)).
...
> **Internal Test Path:** A scalar per-frame fallback path exists inside the `WaveNetA2` fast-path (`src/models/a2/model/static/process.rs`) gated under `#[cfg(test)]`. In production builds (`#[cfg(not(test))]`), the static invariants guarantee that only specialized kernels (CH=3 or CH=8) are executed. In the event of an unforeseen non-standard configuration at runtime, the engine executes a panic-free real-time safe fallback: it records the condition by setting the atomic flag `RT_STATUS_A2_FALLBACK_TRIGGERED`, fires a `debug_assert!(false)` (which verifies in debug builds and compiles away in release), and deterministically silences the layer output without allocating or panicking on the audio thread.
...
| `E2xxx` | Audio / DSP / RT                    | `E2001` PROCESSING_OVERLOAD, `E2100` AUDIO_INIT_FAILED, `E2101` STREAM_ERROR, `E2200` RESAMPLER_BUILD_FAILED, `E2201` RESAMPLER_CHANNEL_FULL, `E2202` INVALID_CABSIM_PARTITION_SIZE, `E2300` RT_PRIORITY_DENIED, `E2301` CPU_AFFINITY_FAILED, `E2302` BACKEND_FAILURE, `E2304` HOST_FORMAT_CONTRACT_VIOLATION — E2100–E2304 reserved for downstream integrations (e.g., plugin wrappers, standalone hosts, offline renderers); never constructed by the core crate (F-19). `E2200` and `E2202` are the exceptions: core-constructed DSP constructor-validation errors (`NamResampler::new_typed`/`new_linear_typed` for an out-of-range sample rate; `ConvEngine::new` with `partition_size == 0`)                                                                                                                                                                                                                                                                                       |
...
The **exceptions** to the "never constructed by the core crate" rule are the DSP constructor-parameter validation errors returned by pure DSP constructors (not host/backend surfaces):

- `E2202` INVALID_CABSIM_PARTITION_SIZE — `ConvEngine::new` when `partition_size == 0`, preventing a panic on the audio thread during transient host init or config reset.
- `E2200` RESAMPLER_BUILD_FAILED — the typed `NamResampler::new_typed` / `new_linear_typed` constructors when a sample rate is outside the supported `4_000..=384_000` Hz window (allocation failures surface separately as `E5000` OUT_OF_MEMORY). Downstream host integrations still emit this code for their own generic rebuild failures.

Each emitted diagnostic includes version, architecture, and timestamp to enable automated diagnostic triage.

### 7.1 Crash Report Diagnostics & Cache Retention Policy

When an unhandled panic occurs within the DSP engine or host lifecycle, the panic hook facility (`src/common/panic_hook.rs`) formats a zero-allocation diagnostic crash dump into a stack-allocated buffer (`[u8; 16384]`) and writes it to `~/.cache/neural-amp-modeler-rs/crash-<unix_ts>-<component>.txt`.

To maintain operational hygiene and prevent disk clutter or confusion in support triage:

- **FIFO Cap (`MAX_CRASH_FILES = 10`):** The panic hook automatically discovers existing `crash-*.txt` files in `~/.cache/neural-amp-modeler-rs/` and removes the oldest entries if the total count exceeds 10.
- **Programmatic Pruning:** Hosts can invoke `DiagnosticBundle::purge_old_reports(max_age_secs)` during off-RT startup to automatically discard resolved crash logs older than a configurable threshold (e.g. 7 days).
- **Manual Maintenance:** Operators and CI runners may safely purge the cache directory at any time:
  ```bash
  rm -f ~/.cache/neural-amp-modeler-rs/crash-*.txt ~/.cache/neural-amp-modeler-rs/crash-*.tmp
  ```
