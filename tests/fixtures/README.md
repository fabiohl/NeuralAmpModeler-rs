...
## Workspace-Level Vendor Mirrors

The upstream C++ mirrors used for cross-reference validation **do not live in
this repository**. They are concentrated in the workspace-level `third-party/`
area (synchronized by `utils/mod-update.sh`), keeping this repo clean:
...
> [!NOTE]
> The third-party base directory resolves to `../third-party/` relative to this
> repo root and can be overridden via the `NAM_THIRD_PARTY_DIR` (or `NAM_CORE_DIR`
> / `NAM_PLUGIN_DIR`) environment variables when the workspace layout differs.
> [!NOTE]
> **`utils/mod-update.sh` syncs both `NeuralAmpModelerCore` and
> `NeuralAmpModelerPlugin`** (including their submodules) into `../third-party/`.
> Run it to set up or update both vendored copies to the pinned versions defined
> in [`variables.env`](../../variables.env).

### To regenerate all fixtures from scratch

```bash
# Remove existing mirrors (optional but recommended for clean regeneration)
rm -rf ../third-party/NeuralAmpModelerCore \
...
(sourced by both `golden_gen_build.sh` and `mod-update.sh`). All goldens
...
All captures and models in `.nam` and `.json` format located under [tests/fixtures/models/](models/) are audited to verify quality, legal provenance, and usefulness in integration tests.
...
#### 3. Non-Distributable Model Management (`third-party/nam_t3k` / `tests/fixtures/models-nondist`)

Due to the legal redistribution restriction of the default `t3k` license in the project's Git, `NeuralAmpModeler-rs` tests can be run locally against protected captures using either the shared workspace directory at `./third-party/nam_t3k/` or a local mirror folder at `tests/fixtures/models-nondist`:
...
| Step | Location                                                | Environment override      | io_helpers equivalent |
|:---- |:------------------------------------------------------- |:------------------------- |:--------------------- |
| 1    | `$NAM_MODELS_DIR/<file>`                                | `NAM_MODELS_DIR`          | step 1                |
| 2    | `$NAM_THIRD_PARTY_DIR/nam_t3k/<file>`                   | `NAM_THIRD_PARTY_DIR`     | step 2                |
| 3    | `tests/fixtures/models-nondist/<file>`                  | —                         | step 3                |
| 4    | `../third-party/nam_t3k/<file>` (relative to crate)     | —                         | step 4                |
| 5    | `tests/fixtures/models/<file>` (default, distributed)   | —                         | step 5                |

> [!IMPORTANT]
> **Skip rule:** a model is **SKIP**ped only if the file is absent in **all** five paths. If the
> file exists at any path, the gen script **must** either render the golden successfully or
> fail hard — never skip silently. The stale "not found at `$MODELS_DIR` or `models-nondist`"
> message from before S2-T1 is gone; if a model exists in `third-party/nam_t3k/` it will be
> found and rendered.

> [!NOTE]
> **Nondist golden policy:** non-distributable models (EVH-5150-Lite, APP-EVH, Boss BD-2,
> SLAMMIN MARSHALL) live under `third-party/nam_t3k/` or `models-nondist/`. When present, the
> gen script renders their goldens into `tests/fixtures/` as usual. The `.nam` files themselves
> must **not** be committed to git (license restriction). The golden `.bin` files follow the
> project policy:
...
- Place your high-quality `t3k` models under `./third-party/nam_t3k/` or mount a symbolic link:

  ```bash
  ln -s /path/to/my/t3k/models tests/fixtures/models-nondist
  ```

- The integration test suite (`tests/models/nondist_validation.rs`) automatically detects `tests/fixtures/models-nondist` or falls back to `./third-party/nam_t3k/`, running a comprehensive verification battery (parsing, determinism, block size invariance, and denormal silence stability) on all captures found. It skips validation gracefully without failure if neither directory is present.
...
See Unversioned Local Mirrors for the pinned upstream commits and
...
> `docs/cpp_parity_map.md` §4.4.3). Fixture + golden permanecem para investigação futura
...
> `wavenet_a2_film_full/lite.nam`. `slimmable_wavenet.nam` **carrega** — disclaimer S4-T1:
...
> validam o motor FiLM. Ver `docs/cpp_parity_map.md` §4.4.3.
