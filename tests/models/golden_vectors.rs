...
/// - Thresholds: SNR ≥ 105 dB, ESR ≤ 3.5e-11 (17.3 dB margin — honest, like Feather CH=8).
///
/// ## Fixture provenance
...
/// Test 8k-2: Slimmable WaveNet metadata parsing (Metadata Structures and Parsing).
///
...
/// Test 8k-3: SlimmableWavenet channel slicing inference and breakpoint validation.
///
/// Validates:
...
fn test_h0_triple_decomposition() {
    ...
    let classification = match (esr_prod_f64 < eps, esr_prod_cpp < eps, esr_f64_cpp < eps) {
        (true, false, false) => "Case A: prod≈f64 ≠ C++ → shared bug / common graph",
        (false, false, true) => "Case B: f64≈C++ ≠ prod → production-only bug",
        (false, true, false) => "Case C: prod≈C++ ≠ f64 → oracle-only bug (unexpected)",
        (false, false, false) => "Case D: all distinct → multiple defects; prioritize cond/FiLM",
...
fn catalog_preflight() {
    ...
    // V1 golden catalog validation — Rust single source of truth.
    ...
    // V2 golden catalog validation — Rust single source of truth.
